...
namespace Netigent.Utils.FileStoreIO
{
	public class FileStoreIOClient : IFileStoreIOClient
	{
		...
		/// <inheritdoc/>
		public async Task<ResultModel> File_Migrate(string fileRef, FileStorageProvider location)
		{
			// Pick the client based on target.
			FileStorageProvider saveTo = location == FileStorageProvider.UseDefault ? _defaultStorage : location;
				...
				foreach (var existingFile in getFileVersions)
				{
					...
					if (existingFile.FileLocation != (int)saveTo)
					{
						try
						{
							...
							messages.Add($"Moved {fileInfo} to {saveTo}");
						}
						catch (Exception ex)
						{
							overallStatus = false;
							messages.Add($"Failed {fileInfo}, to {saveTo}, Error: {ex.Message}");
						}
					}
					else
					{
		...
		private async Task<string> File_UpsertAsync(InternalFileModel fileObject, FileStorageProvider location)
		{
			// Pick the client based on target.
			FileStorageProvider saveTo = location == FileStorageProvider.UseDefault ? _defaultStorage : location;
			...
			fileObject = await SaveFileAsync(fileObject, saveTo);
		...
		private async Task<InternalFileModel> SaveFileAsync(InternalFileModel fileModel, FileStorageProvider saveToLocation)
		{
			...
			if (saveToLocation != FileStorageProvider.Database)
			{
				...
				var client = GetClient(saveToLocation);
				if (client != null)
				{
