...
namespace Netigent.Utils.FileStoreIO
{
    public interface IFileStoreIOClient
    {
        ...
        string File_Upsert(byte[] fileContents, string fullFilename, FileStorageProvider fileLocation = FileStorageProvider.Database, string description = "", string mainGroup = "", string subGroup = "", DateTime created = default);
        ...
        /// <param name="subGroup">(Optional) Used when storing to filesystem will store as \\myserver\filestore\{mainGroup}\{subGroup}\filename.ext</param>
        /// <returns>A unique file-reference for getting the file, if blank issues creating the file</returns>
        Task<string> File_Upsert(IFormFile file, FileStorageProvider fileLocation = FileStorageProvider.Database, string description = "", string mainGroup = "", string subGroup = "");
        ...
        /// <param name="fileRef">FileRef assigned e.g. _$5352532555325</param>
        /// <returns></returns>
        bool File_Delete(string fileRef);
